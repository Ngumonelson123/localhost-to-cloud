...
> **Stack:** Node.js API → Docker → ECR → ECS on EC2 + ALB → RDS PostgreSQL → Secrets Manager → S3 → CI/CD  
> **Presented at:** Saturday Webinar Session
...
Application Load Balancer (ALB — public subnets)
    │  /ui        → serves frontend (index.html from Express)
...
| Tier | Service | Notes |
|------|---------|-------|
| Presentation | Express `/ui` + S3 | Frontend served from ECS, S3 for storage |
...
├── app/
│   ├── public/
│   │   └── index.html        # Frontend (served at /ui via Express)
...
|----------|-------------|
| `/ui` | Demo frontend — tests all endpoints live |
